...
public record CommonConfigSyncPayload(CommonNetwork.Identifier<CommonConfigSyncPayload> identifier, net.minecraft.resources.Identifier commonConfigStorage, CompoundTag configTag) implements CommonNetwork.Payload {
    ...
    public static CommonConfigSyncPayload of(CommonNetwork.Identifier<CommonConfigSyncPayload> identifier, FactoryConfig.StorageHandler handler, FactoryConfig<?> config) {
        return new CommonConfigSyncPayload(identifier, FactoryConfig.COMMON_STORAGES.getKey(handler), (CompoundTag)handler.encodeConfigs(Map.of(config.getKey(), config), NbtOps.INSTANCE));
    }
